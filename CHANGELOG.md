## [1.1.50] - 2026-04-29
### Improved
- **Global**: Added shared HLS quality expansion so master manifests can be exposed as multiple stream variants when the source provides 2160p/1080p/720p and similar ladders.
- **Global**: Normalized quality labels across providers to keep the player list consistent on Nuvio mobile and TV.
...
### Fixed
- **Global**: Added fake-direct filtering to reject known test/sample videos such as Big Buck Bunny before they reach the player.
- **Uqload**: Improved direct-link resolution for `uqload` embeds and kept the working fallback domains.
- **Frenchstream / Global**: Tightened resolver heuristics so only playable media URLs are returned when a host exposes multiple candidates.
...
### Added
- **New Provider**: Added [Sekai.one](https://sekai.one) integration. This new provider focuses on large anime series and offers lightning-fast video streaming by generating direct `.mp4` URLs without heavily relying on iframes or third-party resolvers. Fully compliant with Nuvio specs and armSync episode tracking.

## [1.1.27] - 2026-02-27

### Fixed
- **AnimesUltra Episode Mapping**: Fixed a critical extraction bug where the provider returned incorrect episodes instead of the requested one. Redesigned the parsing logic to associate the episode's `data-number` payload with its specific video server boxes instead of relying on a flawed chronological server index.
- **VoirAnime VF Coverage**: Repaired VF stream retrieval for Season 1. Added specific fallback slugs (`-vf`, `-1-vf`) and removed an early-exit bug in the loop that stopped the script from continuing once the VOSTFR version was found.

## [1.1.26] - 2026-02-27

### Fixed
- **Player Playback Crashes**: Fixed ExoPlayer exception (`23003` / `error_code_parsing_container_unsupported`) affecting `AnimesUltra` and `JetAnimes`. The extractors were incorrectly bypassing the `isDirect` filter, pushing raw HTML web pages to the player instead of resolving them strictly into direct video streams (.mp4, .mkv, .m3u8).

## [1.1.25] - 2026-02-27

### Changed
- **Global Provider Update**: Improved the stream sorting algorithm across all providers to prioritize French dubs (VF) over Japanese with French subtitles (VOSTFR). When both versions are available, VF will now be the first choice.

## [1.1.24] - 2026-02-27

### Added
- **New Provider**: Added [AnimesUltra](https://animesultra.org) to support parsing and extracting direct video streams.
- **New Provider**: Added [JetAnimes](https://on.jetanimes.com) (marked with a warning due to heavy proxying/redirects).
- **Core HTTP Module**: Standardized `http.js` across all newly added sources to bypass Cloudflare 403 blocks in the Hermes engine.
