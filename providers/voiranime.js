 ...
 * Generated: 2026-03-06T10:06:08.498Z
 */
  ...
  return __async(this, null, function* () {
    ...
    for (const slug of allSlugs) {
      ...
      try {
        ...
        validPredictions.push({ title: title + (slug.includes("vostfr") ? " VOSTFR" : slug.includes("vf") ? " VF" : ""), url });
      } catch (e) {
      }
    }
    ...
    try {
      ...
      for (const bs of baseSlugsFromSearch) {
        const seasonSlugs = season === 1 ? [bs, `${bs}-vf`, `${bs}-1`, `${bs}-1-vostfr`, `${bs}-1-vf`] : [`${bs}-${season}`, `${bs}-${season}-vostfr`, `${bs}-${season}-vf`, `${bs}-saison-${season}`];
