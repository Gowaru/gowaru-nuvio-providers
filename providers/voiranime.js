...
<<<<<<< HEAD
 * Generated: 2026-08-27T16:17:12.115432072Z
=======
 * Generated: 2026-08-26T16:18:54.027134Z
>>>>>>> origin/main
 */
var __provider = (() => {
  ...
  function fetchHostEmbed(host, episodeUrl) {
    return __async(this, null, function* () {
      try {
        ...
        console.warn(`[VoirAnime] fetchHostEmbed(${host}) failed: ${err == null ? void 0 : err.message}`);
        return null;
      }
    });
  }
      ...
      try {
        ...
        if (filteredHosts.length === 0) {
          ...
          $("iframe").each((_, el) => {
            ...
            if (src.startsWith("http") && !src.includes("voiranime.com") && classifyEmbed(src) !== "placeholder") {
              iframe = src;
              return false;
            }
          });
        ...
        const candidates = embedUrls.map((u) => ({ url: u, cls: classifyEmbed(u) })).filter((e) => e.cls !== "placeholder");
        ...
        for (const cand of candidates) {
          ...
          try {
            ...
            console.warn(`[VoirAnime] Resolve failed for ${embedDomain(cand.url)}: ${e == null ? void 0 : e.message}`);
            unresolved.push(cand);
          }
        }
    ...
    return __async(this, arguments, function* (tmdbId, mediaType, season, episode, options = {}) {
      ...
      if (SEARCH_CACHE.has(cacheKey) && Date.now() - SEARCH_CACHE.get(cacheKey).ts < SEARCH_CACHE_TTL) {
        ...
        if (allTitles.length > 0 && !isBudgetExhausted(startTime, BUDGET_MS)) {
          ...
          const validUrls = yield batchProbe(limitedUrls, 5, 0);
          if (validUrls.length > 0) {
            for (const url of validUrls) {
              const urlSlug = (_c = (_b = url.match(/\/anime\/([^/]+)\/$/)) == null ? void 0 : _b[1]) == null ? void 0 : _c.replace(/-vf$/, "");
        ...
        if (matches.length === 0) {
          for (const title of allTitles) {
            if (isBudgetExhausted(startTime, BUDGET_MS)) break;
            const result = yield searchAnime(title, effectiveSeason, (_d = titles._metadata) == null ? void 0 : _d.year);
  ...
  var import_cheerio_without_node_native2, BASE_URL, PAGE_TIMEOUT, HOST_TIMEOUT, BUDGET_MS, PROBE_TIMEOUT, SEARCH_CACHE, SEARCH_CACHE_TTL, slugProbeCache, UNRESOLVABLE_EMBEDS, PLACEHOLDER_IFRAMES, SPINOFF_KEYWORDS, MAX_DIRECT_STREAMS;
  var init_extractor = __esm({
    "src/voiranime/extractor.js"() {
      ...
      UNRESOLVABLE_EMBEDS = ["voe.", "streamhide.", "gn1r5n.", "parklogic", "ds2play", "dood.", "bigwar5"];
      PLACEHOLDER_IFRAMES = ["youtube.com/embed", "youtu.be/", "facebook.com/plugins", "twitter.com/i/videos", "ok.ru/videoembed"];
