 ...
 * Generated: 2026-08-26T15:16:20.508380479Z
 */
var __provider = (() => {
  ...
  function resolveFsvidVidzy(url) {
    return __async(this, null, function* () {
      try {
        const embedDomain2 = (url.match(/^https?:\/\/([^/]+)/) || [])[1] || "";
        const embedRef = embedDomain2 ? `https://${embedDomain2}/` : "https://fsvid.lol/";
        const res = yield safeFetch(url, { headers: { Referer: embedRef } });
        if (!res) return { url };
        let html = yield res.text();
        if (html.includes("p,a,c,k,e,d") || html.includes("eval(function")) html = unpack(html);
        let videoUrl = null;
        const hostname = embedDomain2 ? embedDomain2.split("/")[0] : (embedRef.split("//")[1] || "").replace(/\//g, "");
    ...
    return __async(this, arguments, function* (url, options = {}) {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
          ...
          if (attempt < maxRetries && (((_c = e.message) == null ? void 0 : _c.includes("fetch failed")) || ((_d = e.message) == null ? void 0 : _d.includes("NetworkError")) || ((_e = e.message) == null ? void 0 : _e.includes("timeout")) || ((_f = e.message) == null ? void 0 : _f.includes("Cloudflare")))) {
            continue;
          }
  ...
  var HEADERS2, rateLimit, _currentSignal, DOMAIN, RETRY_DELAYS2;
  var init_http = __esm({
    "src/french-anime/http.js"() {
      init_resolvers();
      HEADERS2 = {
  ...
  function probeUrl(url) {
    return __async(this, null, function* () {
      ...
      const res = yield safeFetch(url, { method: "GET", timeout: PROBE_TIMEOUT });
      if (!res || !res.ok) {
  ...
  function batchProbe(urls, batchSize = 5) {
    return __async(this, null, function* () {
      const results = [];
  ...
  function cleanSlug(slug) {
    return slug.replace(/-(?:1st|2nd|3rd|4th|5th)-season$/, "").replace(/-(?:season|saison)-?\d+$/, "").replace(/-s\d+$/, "").replace(/-{2,}/g, "-").replace(/^-|-$/g, "");
  }
  function dleSearch(query, season) {
    return __async(this, null, function* () {
      try {
        ...
        console.warn(`[FrenchAnime] DLE search failed: ${e == null ? void 0 : e.message}`);
        return [];
      }
    });
  }
  function searchAnime(title, season = 1, year) {
    return __async(this, null, function* () {
      ...
      if (season > 1 && baseSlug.length > 3 && !isProbeBudgetExhausted()) {
        ...
        const validUrls = yield batchProbe(seasonUrls, 2);
      ...
      if (baseSlug.length > 3 && !isProbeBudgetExhausted()) {
        ...
        const validUrls = yield batchProbe(urls, 2);
        if (validUrls.length > 0) {
          validUrls.forEach((url) => {
            const lang = url.includes("/animes-vf/") ? "VF" : "VOSTFR";
      ...
      if (results.length === 0 && !isProbeBudgetExhausted()) {
        const searchResults = yield dleSearch(title, season);
        for (const r of searchResults) {
          if (!results.some((ex) => ex.url === r.url)) {
            results.push(__spreadValues({}, r));
          }
        }
        if (results.length > 0) return results;
      }
          ...
          for (const r of searchResults) {
            if (r.status === "fulfilled") {
              for (const res of r.value) {
                if (!results.some((ex) => ex.url === res.url)) results.push(__spreadValues({}, res));
              }
            }
          }
          if (results.length > 0) return results;
    ...
    for (const sel of EPISODE_SELECTORS) {
      $(sel).each((i, el) => {
        ...
        for (const pattern of epPatterns) {
          const regex2 = new RegExp(`(?:^|[^0-9])${pattern}(?:$|[^0-9])`, "i");
          if (regex2.test(cleanText) || regex2.test(href)) {
            episodeUrl = href;
            return false;
          }
        }
  ...
  function resolveEpisodeStreams(episodeUrl, lang, streamHeaders, startTime) {
    return __async(this, null, function* () {
      try {
        ...
        if (extraction.type === "episode") {
          const collected = yield Promise.allSettled(
          ...
          for (const url of extraction.urls) {
            if (direct2.length >= MAX_DIRECT_STREAMS) break;
            ...
            try {
              ...
              if (stream && stream.isDirect) direct2.push(stream);
            } catch (e) {
            }
          }
          if (direct2.length > 0) return direct2;
        ...
        const candidates = embedUrls.map((u) => ({ url: u, cls: classifyEmbed(u) })).filter((e) => e.cls !== "dead" && e.cls !== "placeholder");
        candidates.sort((a, b) => {
        ...
        for (const cand of candidates) {
          ...
          try {
            ...
            console.warn(`[FrenchAnime] Resolve failed: ${e == null ? void 0 : e.message}`);
            unresolved.push(cand);
          }
        }
        ...
        console.warn(`[FrenchAnime] resolveEpisodeStreams failed: ${e == null ? void 0 : e.message}`);
        return [];
      }
    });
  }
  function fetchHostEmbed(host, episodeUrl) {
    return __async(this, null, function* () {
      try {
        ...
        console.warn(`[FrenchAnime] fetchHostEmbed(${host}) failed: ${err == null ? void 0 : err.message}`);
        return null;
      }
    });
  }
    ...
    return __async(this, arguments, function* (tmdbId, mediaType, season, episode, options = {}) {
      ...
      if (SEARCH_CACHE.has(cacheKey) && Date.now() - SEARCH_CACHE.get(cacheKey).ts < SEARCH_CACHE_TTL) {
        ...
        if (allSearchTitles.length > 0 && !isBudgetExhausted(startTime, BUDGET_MS)) {
          ...
          const validUrls = yield batchProbe(allUrls, 5);
          if (validUrls.length > 0) {
            for (const url of validUrls) {
              const urlSlug = (_c = (_b = url.match(/\/([^/]+)\.html$/)) == null ? void 0 : _b[1]) == null ? void 0 : _c.replace(/-vf$/, "");
        ...
        if (matches.length === 0) {
          for (const title of allSearchTitles) {
            if (isBudgetExhausted(startTime, BUDGET_MS)) break;
            const result = yield searchAnime(title, effectiveSeason, year);
      ...
      for (const match of matches) {
        ...
        try {
          ...
          let episodeUrl = null;
          if ((extraction == null ? void 0 : extraction.type) === "episode" || (extraction == null ? void 0 : extraction.type) === "link") {
            episodeUrl = match.url;
          } else if ((extraction == null ? void 0 : extraction.type) === "direct_iframes" || (extraction == null ? void 0 : extraction.type) === "direct_video") {
            episodeUrl = match.url;
          } else {
            const $ = import_cheerio_without_node_native2.default.load(html);
            for (const sel of EPISODE_SELECTORS) {
          ...
          const epStreams = yield resolveEpisodeStreams(episodeUrl, lang, streamHeaders, startTime);
          streams.push(...epStreams);
        } catch (e) {
          console.warn(`[FrenchAnime] Match processing failed: ${e == null ? void 0 : e.message}`);
        }
      }
  ...
  var import_cheerio_without_node_native2, PAGE_TIMEOUT, HOST_TIMEOUT, PROBE_TIMEOUT, BUDGET_MS, SEARCH_CACHE, SEARCH_CACHE_TTL, slugProbeCache, RESOLVABLE_HOSTS, DEAD_HOSTS, PLACEHOLDER_IFRAMES, SPINOFF_KEYWORDS, EPISODE_SELECTORS, MAX_DIRECT_STREAMS, MAX_STREAMS;
  var init_extractor = __esm({
    "src/french-anime/extractor.js"() {
      ...
      DEAD_HOSTS = ["streamhide", "parklogic", "ds2play", "bigwar5", "voe.sx"];
      PLACEHOLDER_IFRAMES = [
        "youtube.com/embed",
        "youtu.be/",
        "facebook.com/plugins",
