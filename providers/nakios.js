 ...
 * Generated: 2026-09-13T12:10:35.537435491Z
 */
var __provider = (() => {
  ...
  var PROVIDER_BUDGET_MS, MAX_STREAMS_PER_PROVIDER, MAX_SAFE_FETCH_BODY_BYTES, BUILD_HASH, HAS_NATIVE_CRYPTO, _nodeCrypto, HEADERS, USER_AGENT, BASE_HEADERS, CODEC_PREFERENCE, STRICT_QUALITY_TIERS, DEFAULT_QUALITY_TIER, CODEC_PRIORITY, manifestCache, MANIFEST_CACHE_TTL, FETCH_CACHE_TTL, fetchCache, LANGUAGE_CODE_MAP;
  ...
  var init_http = __esm({
    "src/nakios/http.js"() {
      ...
      BASE_URL = safeConfig("NUVIO_NAKIOS_SITE_URL", "https://nakios.cyou");
      API_BASE = safeConfig("NUVIO_NAKIOS_API_URL", "https://api.nakios.store");
  ...
  function fallbackSearch(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      const { title, year } = yield getTmdbTitleYear(tmdbId, normType);
      if (!title) {
  ...
  function trySearchResults(results, originalTmdbId, mediaType, season, episode, wantTitle, wantYear) {
    return __async(this, null, function* () {
      ...
      const strict = results.filter((r) => {
        ...
        if (wantYear && r.year && Math.abs(parseInt(r.year, 10) - wantYear) > 1) {
          console.log(`[Nakios] Fallback: skip ${r.id} (ann\xE9e ${r.year} \u2260 ${wantYear}\xB11)`);
          return false;
        }
        return true;
      });
      if (strict.length === 0) {
        console.log(`[Nakios] Fallback: aucun r\xE9sultat de m\xEAme type/titre/ann\xE9e \u2014 abandon (\xE9vite le faux-positif)`);
        return null;
      }
