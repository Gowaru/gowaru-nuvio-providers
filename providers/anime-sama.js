 ...
 * Generated: 2026-09-13T12:38:41.120121078Z
 */
var __provider = (() => {
  ...
  var _currentSignal, DOMAIN, rateLimit, HEADERS2;
  var init_http = __esm({
    "src/anime-sama/http.js"() {
      init_resolvers();
      _currentSignal = null;
    ...
    return __async(this, arguments, function* (slug, lang, season, episode, mediaType, altEpisodes = []) {
      ...
      if (mediaType === "movie") {
        for (const seasonPath of ["film", "film2"]) {
          const jsContent = yield fetchJs(slug, seasonPath, lang);
          if (!jsContent) continue;
          const parsed = parseUrls(jsContent);
          if (parsed.length === 0) continue;
          const streams = yield buildStreams(parsed, lang, null, 0);
  ...
  function tryFetchEpisode(slug, lang, season, episode) {
    return __async(this, null, function* () {
      const mainJs = yield fetchJs(slug, `saison${season}`, lang);
      if (mainJs) {
      ...
      const rootJs = yield fetchJs(slug, "", lang);
      if (rootJs) {
      ...
      if (streams.length === 0 && !isAborted(signal) && !isBudgetExhausted(startTime, BUDGET_MS)) {
        ...
        if (mediaType === "movie") {
          for (const t of [...searchTitles]) {
            const seriesTitle = t.replace(/\s*[-–:]?\s*(?:the\s+)?movie\b.*$/i, "").replace(/\s*[-–:]?\s*(?:le\s+)?film\b.*$/i, "").replace(/\s*[-–:]?\s*oav\b.*$/i, "").trim();
