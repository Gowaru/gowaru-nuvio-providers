 ...
 * Generated: 2026-09-13T12:10:35.586435571Z
 */
var __provider = (() => {
  ...
  function parseMaybeTruncatedJson(text) {
    ...
    for (let end = text.lastIndexOf("}"); end > 0; end = text.lastIndexOf("}", end - 1)) {
      try {
        return JSON.parse(closeTruncatedJson(text, end));
      } catch (e) {
        continue;
      }
    }
  ...
  function getAbsoluteEpisode(imdbId, season, episode) {
    return __async(this, null, function* () {
      ...
      if (!res) return null;
      const data = parseMaybeTruncatedJson(yield res.text()) || {};
  ...
  function searchTmdbByTitle(title, mediaType) {
    return __async(this, null, function* () {
      ...
      const url = `${TMDB_API_BASE2}/search/${type}?api_key=${TMDB_API_KEY2}&query=${encoded}`;
      const res = yield safeFetch(url);
      if (!res) return null;
      let data;
      try {
  ...
  function getTMDBTitlesById(_0, _1) {
    return __async(this, arguments, function* (tmdbId, mediaType, opts = {}) {
      ...
      let metadata = null;
      try {
        const mainUrl = `${TMDB_API_BASE2}/${type}/${tmdbId}?api_key=${TMDB_API_KEY2}&language=en-US`;
        const altUrl = `${TMDB_API_BASE2}/${type}/${tmdbId}/alternative_titles?api_key=${TMDB_API_KEY2}`;
        const transUrl = `${TMDB_API_BASE2}/${type}/${tmdbId}/translations?api_key=${TMDB_API_KEY2}`;
  ...
  var TMDB_API_KEY2, TMDB_API_BASE2, METADATA_CACHE, METADATA_TTL, METADATA_MAX, SEASON_SUFFIXES;
  ...
  function getSitemapSlugs() {
    return __async(this, null, function* () {
      return withCache("sitemap_slugs", () => __async(null, null, function* () {
        try {
          const xml = yield fetchText(`${BASE_URL}/sitemap.xml`, { retries: 0 });
          const slugs = [];
          const re = /<loc>([^<]+)<\/loc>/gi;
          let m;
          ...
          console.log(`[Sekai] Sitemap: ${slugs.length} s\xE9ries`);
          return slugs;
        } catch (e) {
  ...
  function evalConcatExpr(expr, constants, loopVar, loopValue) {
    ...
    for (const raw of parts) {
      ...
      if (/^[A-Za-z_$][\w$]*$/.test(p)) {
        ...
        if (loopVar && (p === loopVar || p === "num")) {
          out += String(loopValue);
          continue;
        }
        return null;
      }
  ...
  function extractLastCounters(code, dataObjects) {
    ...
    while ((m = refRe.exec(code)) !== null) {
      const value = (_a = dataObjects.get(m[2])) != null ? _a : m[3] != null ? parseInt(m[3], 10) : null;
  ...
  function parseEpisodeData(code, targetKeys) {
    ...
    for (const [, value] of constants) {
      if (value.includes("sibnet") && value.includes("php")) {
        sibnetBase = value;
        break;
      }
    }
    ...
    for (const st of statements) {
      ...
      if (!value) continue;
      if (!families.has(st.name)) families.set(st.name, /* @__PURE__ */ new Map());
  ...
  function parseJsPlayerFamily(code, targetKeys) {
    const out = [];
    const cleaned = stripJsComments(code);
    const episodesMatch = cleaned.match(/AVAILABLE_EPISODES\s*=\s*Object\.freeze\(\s*(\[[\s\S]*?\])\s*\)/) || cleaned.match(/AVAILABLE_EPISODES\s*=\s*(\[[\s\S]*?\])\s*;/);
    ...
    const numbers = episodes.map((e) => parseInt(e && e.number, 10)).filter((n) => isFinite(n));
  ...
  function discoverSubPages(html, slug) {
    ...
    while ((m = hrefRe.exec(html)) !== null) {
      ...
      if (!pages.has(url) || detected !== null && ((_a = pages.get(url)) == null ? void 0 : _a.index) == null) pages.set(url, { index: detected });
    }
    ...
    while ((m = jqAnchorRe.exec(html)) !== null) {
      ...
      if (!pages.has(norm) || detected !== null && ((_b = pages.get(norm)) == null ? void 0 : _b.index) == null) pages.set(norm, { index: detected });
    }
    const found = [...pages.keys()];
    if (!found.length) {
      console.warn(`[Sekai] Sub-pages: aucune sous-page /${slug}/ ou jq li\xE9e d\xE9tect\xE9e`);
    }
  ...
  function buildStreams(candidates, signal, startTime) {
    return __async(this, null, function* () {
      ...
      for (const cand of ordered) {
        ...
        const stream = {
          ...
          language: "VOSTFR",
          type: isDirectMedia(cand.url) ? /\.m3u8/i.test(cand.url) ? "hls" : "mp4" : void 0,
          headers: { Referer: `${BASE_URL}/`, "User-Agent": USER_AGENT2 }
        };
        ...
        try {
          ...
          if (resolved && resolved.url && resolved.isDirect) {
            streams.push(__spreadProps(__spreadValues(__spreadValues({}, stream), resolved), { quality: stream.quality, language: stream.language }));
          }
        } catch (e) {
  ...
  function extractStreams(_0, _1, _2, _3) {
    return __async(this, arguments, function* (tmdbId, mediaType, season, episodeNum, options = {}) {
      ...
      const [ep, absoluteEp] = yield resolveTargetEpisodes(tmdbId, isTv ? "tv" : mediaType, season, episodeNum, { startTime, budgetMs: BUDGET_MS });
      const targetEpisode = absoluteEp || ep;
      console.log(`[Sekai] ${mediaType} S${season}E${episodeNum} \u2192 \xE9pisode site ${targetEpisode}`);
      const targetKeys = [targetEpisode, ep].filter((v, i, arr) => v != null && arr.indexOf(v) === i);
      const slugs = yield getSitemapSlugs();
      const candidates = findSlugCandidates(slugs, titles);
      if (!candidates.length) {
        console.log(`[Sekai] Aucun slug du sitemap ne correspond \xE0 "${titles[0]}"`);
        return [];
      }
      ...
      for (const candidate of candidates.slice(0, 3)) {
        ...
        let html = "";
        try {
          html = yield fetchText(seriesUrl, { retries: 1 });
        } catch (e) {
        ...
        let hits = null;
        yield fetchBatch(requests, (url, opts) => __async(null, null, function* () {
          if (isAborted(signal)) return null;
          let content = html;
          if (!(opts == null ? void 0 : opts.isFirst)) {
            try {
              content = yield fetchText(url, { retries: 0 });
            } catch (e) {
              return null;
            }
          }
        ...
        if (hits && hits.length) {
          console.log(`[Sekai] \xC9pisode ${targetEpisode} trouv\xE9 (${hits.length} source(s))`);
          const streams = yield buildStreams(hits, signal, startTime);
          if (streams.length) return streams;
        }
        console.log(`[Sekai] \xC9pisode ${targetEpisode} introuvable pour ${candidate.slug}`);
      }
      return [];
    });
  }
  ...
  var BASE_URL, BUDGET_MS, MAX_EMBED_RESOLVES, withCache, DEAD_HOSTS, USER_AGENT2, FAMILY_QUALITY;
  var init_extractor = __esm({
    "src/sekai/extractor.js"() {
      ...
      BUDGET_MS = 4e4;
      MAX_EMBED_RESOLVES = 3;
      withCache = createCache("sk", "Sekai", { successTtl: 10 * 6e4, failureTtl: 3e4, maxSize: 200 });
      DEAD_HOSTS = ["mugiwara.xyz", "upvid.co", "opvid.org", "jetload"];
