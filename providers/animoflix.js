 ...
 * Generated: 2026-06-23T17:53:10.456190285Z
 */
var __provider = (() => {
  ...
  function _extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      for (const candidate of ranked) {
        const verifyHtml = yield fetchWithRetry(() => fetchText(`${BASE_URL}/anime/${candidate.slug}/`, { timeout: TIMEOUT }));
        const $v = import_cheerio_without_node_native.default.load(verifyHtml);
        ...
        if (!pageTitle2) {
          ...
          if (!matchesTmdb) {
            console.warn(`[AnimoFlix] Page title mismatch: "${pageTitle2}" doesn't match any TMDB title \u2014 rejecting slug ${candidate.slug}`);
            badSlugs.add(candidate.slug);
            matchOk = false;
          }
        }
      ...
      const animeDetailHtml = yield fetchWithRetry(() => fetchText(`${BASE_URL}/anime/${slug}/`, { timeout: TIMEOUT }));
      ...
      for (const targetSeason of fallbackSeasons) {
        ...
        const seasonHtml = yield fetchWithRetry(() => fetchText(seasonPageUrl, { timeout: TIMEOUT }));
