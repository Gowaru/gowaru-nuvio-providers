 ...
 * Generated: 2026-08-28T14:42:08.940128875Z
 */
var __provider = (() => {
  ...
  function scoreMatch(resultTitle, searchTitle, SCORES2) {
    ...
    if (nr.includes(nt) || nt.includes(nr)) {
      ...
      if (extra > 0) {
        return Math.max(SCORES2.STRONG_MATCH - Math.min(extra * 25, SCORES2.STRONG_MATCH - SCORES2.MIN_MATCH - 5), 0);
      }
      return SCORES2.STRONG_MATCH;
    }
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (resolved && resolved.url) {
          ...
          const isPlayable = urlLower.includes(".mp4") || urlLower.includes(".m3u8") || urlLower.includes("/hls/") || urlLower.includes("master.m3u8");
          if (isPlayable) return __spreadProps(__spreadValues({}, resolved), { isDirect: true });
          return __spreadProps(__spreadValues({}, resolved), { isDirect: false });
        }
        return null;
      } catch (e) {
        return null;
  ...
  var import_cheerio_without_node_native, ARM_API, CINEMATA_API, CACHE, TITLE_NOISE_WORDS;
      ...
      for (const match of matches) {
        try {
          ...
          for (const a of attempts) {
            try {
              const result = yield extractEpisodeStreams(a.match, a.season, a.episode, subType);
              if (result.length > 0) {
