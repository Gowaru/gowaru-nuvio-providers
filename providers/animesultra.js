...
var __provider = (() => {
  ...
  function detectSeason(title, url) {
    ...
    const urlSeason = (_a = url.match(/saison[-\s]*(\d+)/i)) == null ? void 0 : _a[1];
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      if (firstTitle) yield trySearch(firstTitle);
      const frenchTitles = titlesOrdered.filter(
        (t) => t !== firstTitle && t.length < 60 && (/^L['\u2019]/i.test(t.trim()) || /[àâéèêëîïôùûüç]/i.test(t))
      );
      ...
      if (streams.length === 0 && season && matches.length > 1) {
        ...
        for (const m of matches) {
          ...
          const pNum = parseInt((_a = m.title.match(/(?:partie|part)\s*(\d+)/i)) == null ? void 0 : _a[1], 10);
          if (sNum === season) {
            const nId = (_b = m.url.match(/\/(\d+)-/)) == null ? void 0 : _b[1];
            if (nId) {
              const html = yield getFullStory(nId);
              if (html) {
                const $c = import_cheerio_without_node_native.default.load(html);
        ...
        for (const sp of seasonParts) {
          const $c = import_cheerio_without_node_native.default.load(sp.html);
          const epHrefs = [];
          $c(".ep-item").each((i, el) => {
  ...
  var import_cheerio_without_node_native, BASE_URL, SEASON_PATTERNS;
