 ...
 * Generated: 2026-08-26T00:35:44.457544385Z
 */
var __provider = (() => {
  ...
  var PROVIDER_BUDGET_MS, HEADERS, USER_AGENT, BASE_HEADERS, _atob, CODEC_PREFERENCE, TV_BUDGET_MS, STRICT_QUALITY_TIERS, DEFAULT_QUALITY_TIER, CODEC_PRIORITY, manifestCache, MANIFEST_CACHE_TTL, FETCH_CACHE_TTL, fetchCache, LANGUAGE_CODE_MAP, KNOWN_HOST_NAMES, peeledUrls, AD_IFRAME_PATTERNS, VIDEO_IFRAME_SCORE, BASE_URL_FORBIDDEN_PATTERN;
  ...
  var _currentSignal, DOMAIN, rateLimit, BASE_URL, HEADERS2;
  var init_http = __esm({
    "src/anime-ultime/http.js"() {
      init_resolvers();
      _currentSignal = null;
      DOMAIN = "v5.anime-ultime.net";
      rateLimit = createProviderRateLimiter(250, 0.3);
      BASE_URL = "https://v5.anime-ultime.net";
      HEADERS2 = {
  ...
  function fetchSeriesPage(url, signal) {
    return __async(this, null, function* () {
      const html = yield fetchText(url, { signal });
  ...
  function fetchEpisodeFocus(url, signal) {
    return __async(this, null, function* () {
      const html = yield fetchText(url, { signal });
  ...
  function fetchPlayer(serieId, focusFile, signal) {
    return __async(this, null, function* () {
      const data = yield withTimeout(
  ...
  function resolveSeries(page, mediaType, episodeNums, lang, signal, startTime) {
    return __async(this, null, function* () {
      if (!page.serieId) return [];
      if (mediaType === "movie") {
        const focus2 = page.directFocus || (page.epLinks.length > 0 ? yield fetchEpisodeFocus(BASE_URL + "/" + page.epLinks[0].href, signal) : null);
        if (!focus2) return { streams: [], licensed: false };
        const player2 = yield fetchPlayer(page.serieId, focus2, signal);
        if (player2 && player2.__licensed) return { streams: [], licensed: true };
        const mp42 = extractMp4Url(player2);
        if (mp42) {
          const filmLang = page.epLinks.length > 0 ? page.epLinks[0].lang : "vf";
          const stream = yield buildStream(player2, mp42, filmLang, startTime);
          return { streams: stream ? [stream] : [], licensed: false };
        }
        return { streams: [], licensed: false };
      }
      const anchorLink = page.epLinks.find((l) => episodeNums.includes(l.num) && l.lang === lang) || page.epLinks.find((l) => episodeNums.includes(l.num)) || page.epLinks.find((l) => l.lang === lang) || page.epLinks[0];
      if (!anchorLink) return { streams: [], licensed: false };
      const focus = yield fetchEpisodeFocus(BASE_URL + "/" + anchorLink.href, signal);
      if (!focus) return { streams: [], licensed: false };
      const player = yield fetchPlayer(page.serieId, focus, signal);
      ...
      const streams = [];
      const pushIfValid = (p, m, labelLang) => __async(null, null, function* () {
        if (!m) return;
        const s = yield buildStream(p, m, labelLang || lang, startTime);
        if (s) streams.push(s);
      });
      const isAnchorTarget = episodeNums.includes(anchorLink.num);
      if (isAnchorTarget) {
        yield pushIfValid(player, mp4, anchorLink.lang);
      }
      if (episodeNums.length > 0 && (!isAnchorTarget || streams.length === 0)) {
        ...
        if (targetEntry && targetEntry.id) {
          const targetPlayer = yield fetchPlayer(page.serieId, targetEntry.id, signal);
          yield pushIfValid(targetPlayer, extractMp4Url(targetPlayer));
        }
      }
      return { streams, licensed: false };
    });
  }
  function buildStream(player, mp4, lang, startTime) {
    return __async(this, null, function* () {
  ...
  function extractStreams(_0, _1, _2, _3) {
    return __async(this, arguments, function* (tmdbId, mediaType, season, episode, options = {}) {
      ...
      const targetEpisodes = yield resolveTargetEpisodes(tmdbId, mediaType, season, episode, { startTime, budgetMs: BUDGET_MS });
      ...
      for (const title of titlePool.slice(0, MAX_SEARCH_TITLES)) {
        ...
        const found = yield findSeriesForTitle(title, mediaType, effectiveSeason, signal, startTime);
        if (!found) continue;
        for (const l of langOrder) {
          if (isAborted(signal) || isBudgetExhausted(startTime, BUDGET_MS)) break;
          const resolved = yield resolveSeries(found.page, mediaType, episodeNums, l, signal, startTime);
  ...
  function findSeriesForTitle(title, mediaType, season, signal, startTime) {
    return __async(this, null, function* () {
      const results = yield searchSite(title, signal);
      const scored = results.map((r) => __spreadProps(__spreadValues({}, r), { score: scoreSearchResult(r, title, season) })).sort((a, b) => b.score - a.score);
      for (const r of scored) {
        ...
        try {
          ...
          const page = yield fetchSeriesPage(pageUrl, signal);
      ...
      if (!isAborted(signal) && !isBudgetExhausted(startTime, BUDGET_MS)) {
        ...
        if (slug) {
          ...
          for (const cand of candidates) {
            ...
            try {
              const page = yield fetchSeriesPage(url, signal);
              if (page.serieId) return { page };
            } catch (e) {
            }
          }
  ...
  var import_cheerio_without_node_native2, MAX_SEARCH_TITLES, BUDGET_MS, PLAYER_TIMEOUT_MS;
  ...
  // src/anime-ultime/index.js
  var require_index = __commonJS({
    "src/anime-ultime/index.js"(exports, module) {
      init_extractor();
      init_resolvers();
